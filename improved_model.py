...
note_lstm_bw = tf.contrib.rnn.BasicLSTMCell(note_lstm_state_size)
note_state_bw = tf.zeros([batch_size * sample_length, note_lstm_state_size])
...
note_lstm_2_bw = tf.contrib.rnn.BasicLSTMCell(note_lstm_state_size_2)
note_state_2_bw = tf.zeros([batch_size * sample_length, note_lstm_state_size_2])
...
# train_step = tf.train.AdadeltaOptimizer(learning_rate=0.01, rho=0.95, epsilon=0.95).minimize(total_loss)
...
# sess = tf.Session(config=tf.ConfigProto(log_device_placement=True))    
    with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
        ...
        for epoch_idx in range(num_epochs):
            ...
            print("LOSS FOR EPOCH #%d (mean cross entropy): %f" % (epoch_idx, _total_loss / 2))
            if epoch_idx % 10 == 9:
